...
/**
 * Muestra una serie al mejor de 3.
 *
 * Convención de localía guardada en Firestore:
 *   G1: teamA es local → homeScore = teamA
 *   G2: teamB es local → homeScore = teamB (el score de teamA está en awayScore)
 *   G3: teamA es local → homeScore = teamA
 */

// Escudo SVG placeholder para equipos aún no definidos
...
function GameRow({ label, homeLabel, awayLabel, homeTeam, awayTeam, homeScore, awayScore, homeWon, played, isPending }) {
    const { theme } = useTheme();
    const homeShort = homeTeam
        ? (teamShortNames[homeTeam] ?? homeTeam)
        : homeLabel ?? "–";
    const awayShort = awayTeam
        ? (teamShortNames[awayTeam] ?? awayTeam)
        : awayLabel ?? "–";
    ...
    return (
        ...
        >
            {/* Label del partido */}
            <span
                className="text-xs font-black uppercase tracking-wider w-7 shrink-0"
                style={{ color: "#A90000" }}
            >
                {label}
            </span>
            ...
            <div className="flex items-center gap-1 flex-1 min-w-0 justify-end">
                <span
                    className="text-xs font-bold uppercase truncate hidden sm:block"
            ...
            <div className="flex items-center gap-1 shrink-0">
                <span
                    className="text-sm font-black w-7 text-right"
                ...
                <span
                    className="text-sm font-black w-7 text-left"
                    style={{ color: played ? (!homeWon ? "#A90000" : theme.textSecondary) : theme.textMuted }}
                >
                    {played ? awayScore : "–"}
                </span>
...
export default function PlayoffSeriesCard({ series }) {
    const { theme } = useTheme();
    const {
    ...
    // ¿Los equipos aún no están definidos?
    ...
    return (
        <div
            ...
            style={{
                ...
                border: `1px solid ${
                    seriesOver ? "#A9000033" : isPending ? theme.border : theme.borderStrong
                }`,
        ...
        >
            ...
            {isPending && (
                <div
                    className="text-xs font-semibold uppercase tracking-widest text-center py-1 rounded-lg"
            ...
            {/* Equipos y marcador de serie */}
            <div className="flex items-center gap-2">
                <TeamHeader
                    team={teamA}
                    seed={seedA}
                    wins={winsA}
                    isWinner={seriesWinner === teamA}
                    label={labelA}
                    isPending={isPending}
                />

                <div className="flex flex-col items-center gap-1 shrink-0">
                    <span className="text-xs font-black uppercase tracking-widest" style={{ color: theme.textMuted }}>
                        Serie
                    </span>
                ...
                <TeamHeader
                    ...
                    seed={seedB}
                    wins={winsB}
                    isWinner={seriesWinner === teamB}
                    label={labelB}
                    isPending={isPending}
                />
            </div>

            {/* Detalle de partidos — siempre visible, con placeholders si no hay datos */}
